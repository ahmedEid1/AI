    ...
    def add_knowledge(self, cell, count):
        ...
        for i in range(cell[0] - 1, cell[0] + 2):
            for j in range(cell[1] - 1, cell[1] + 2):
                ...
                if 0 <= i < self.height and 0 <= j < self.width:
                    # check the cell is not already determined
        ...
        for s in self.knowledge:
            if s.known_safes() is not None:
                self.safes = self.safes.union(s.known_safes())
            if s.known_mines() is not None:
                self.mines = self.mines.union(s.known_mines())

        for s1 in self.knowledge:
            for s2 in self.knowledge:
                if s1.cells.issubset(s2.cells):
                    set_cells = s2.cells - s1.cells
                    set_count = s2.count - s1.count
                    s3 = Sentence(set_cells, set_count)
